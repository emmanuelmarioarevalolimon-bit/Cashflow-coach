...
with tempfile.TemporaryDirectory(prefix='c1-web-browser-') as tmp:
    ...
    try:
      with sync_playwright() as p:
        ...
        page.get_by_role('link',name='Predicción avanzada',exact=True).click(no_wait_after=True);page.wait_for_url('**/predictions',wait_until='domcontentloaded',timeout=60000)
        ...
        page.locator('#stockoutProduct').select_option(index=1);page.locator('#stockoutDate').fill('2026-09-11');page.locator('#stockoutUnits').fill('8');page.locator('#stockoutNote').fill('Cliente solicitó producto sin existencia.');page.locator('#stockoutForm button').click()
        expect(page.locator('#catalogMessage')).to_contain_text('Nueva demanda estimada',timeout=10000)
        expect(page.locator('#catalogList')).to_contain_text('1 reportes')
        page.locator('#workSchedule summary').click();page.locator('#nonWorkingDates').fill('2026-09-16 | Cierre de inventario');page.locator('#workScheduleForm button').click()
        expect(page.locator('#catalogMessage')).to_contain_text('Calendario laboral guardado',timeout=10000)
        results.append('Compra, catálogo persistente y estimación por faltantes: OK')
